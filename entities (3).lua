...
mobs:register_mob("dementedscript:blimey", {
    ...
    drops = {
        ...
        {name = "default:diamond_block", chance = 1, min = 5, max = 10},
...
-- Custom behavior: Remove torches nearby (and drop them)
    ...
    do_custom = function(self, dtime)
        ...
        -- Torch node names (Minetest Game)
        ...
        -- search for torches near Blimey
        ...
        for _, npos in ipairs(nodes) do
            ...
            -- Remove the torch (replace with air)

            minetest.set_node(npos, {name = "air"})

            -- Drop the torch item naturally at the position

            minetest.add_item(npos, node.name)

            -- Sound + log for effect

            minetest.sound_play("torch_extinguish", {
            ...
            minetest.log("action", "[Blimey] Broke torch and dropped it at " .. minetest.pos_to_string(npos))
